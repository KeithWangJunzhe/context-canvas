...
- 2026-08-05 update: bundle Markdown now presents `Reading Rules`, a typed/id'd `Context Map`, `Context Details`, and a separate `Secondary Relations` section. `related` edges are no longer treated as hierarchy; JSON relations expose `kind: flow|related`.
